...
public class ActionCheckInTests {
	...
	public void InsteadCheckinNotExistPluginName() throws NotExistPluginNameException {
...
//		UserEntity userEntityPlayerAfterPost = this.userSerivce.getByKey(userEntityPlayer.getKey());
	...
	public void checkinOnUnAuthorizedElement() throws UnAuthorizedActionOnElementException {

		// GIVEN:
		// 1. create user Entity type manager and save it in DB
		// 2. create element Entity set its type to LadderSnake and save it in DB
		// 3. create a user Entity type player will do Check in for the Ladder Snake
		// WHEN:
		// activate action with them
		// THEN:
		// 1. Do Check in for the board
		...
		if (actionEntityAfterPost != null) {
			if (actionEntityAfterPost.getMoreAttributes() != null) {
				if (actionEntityAfterPost.getMoreAttributes().containsKey("error")) {
					if (actionEntityAfterPost.getMoreAttributes().get("error").equals("Checkin not on Board or Tile")) {
						// 3. except UnAuthorizedActionOnElementException because can't do check in to
						// LadderSnake
	...
	@Test(expected = DoCheckInOnCheckInException.class)
	public void checkinOnCheckIn() throws DoCheckInOnCheckInException {
		...
		// THEN:
		// 1.Do Check in for the Tile
		// 2.Do Check in Second Time on the board
		// 2. check if the addition of new key and value to map successes and really
		// saved in DB
		...
		// THEN:
		// 1.Do Check in First Time on the board
		this.restTemplate.postForObject(baseUrl, actionBoundary, ActionBoundary.class);

		// 2.Do Check in Second Time on the board
		this.restTemplate.postForObject(baseUrl, actionBoundary, ActionBoundary.class);

		// 3. check if the addition of new key and value to map successes and really
		// saved in DB
				...
				if (actionEntityAfterPost.getMoreAttributes().get(userEntityPlayerAfterPost.getKey())
					...
					if (actionEntityAfterPost.getMoreAttributes().containsKey("error")) {
						...
						if (actionEntityAfterPost.getMoreAttributes().get("error")
								.equals("Can't do check in when user is already check in status")) {
							System.err.println("222");
							throw new DoCheckInOnCheckInException();
						}
	...
	public void checkinDoneByManager() throws UnAuthorizedException {
		...
		// activate action with them
		ActionEntity actionEntity = new ActionEntity(elementEntity.getElementSmartspace(), elementEntity.getElementId(),
				"2019b.marlenba", "ggwp@gmail.com", "CheckIn", new Date(), new LinkedMultiValueMap());
		ActionBoundary actionBoundary = new ActionBoundary(actionEntity);
		...
		if (actionEntityAfterPost != null) {
			if (actionEntityAfterPost.getMoreAttributes() != null) {
				if (actionEntityAfterPost.getMoreAttributes().containsKey("error")) {
					if (actionEntityAfterPost.getMoreAttributes().get("error").equals("Only player can do checkin")) {
						// 3. except UnAuthorizedActionOnElementException because can't do check in to
						// LadderSnake
						throw new UnAuthorizedException();
						// addition of the check in plug-in to more attributes success
	...
	@Test(expected = UnAuthorizedException.class)
	public void checkinDoneByAdmin() throws UnAuthorizedException {
		...
		// activate action with them
		ActionEntity actionEntity = new ActionEntity(elementEntity.getElementSmartspace(), elementEntity.getElementId(),
				"2019b.marlenba", "ggadmin@gmail.com", "CheckIn", new Date(), new LinkedMultiValueMap());
		ActionBoundary actionBoundary = new ActionBoundary(actionEntity);
System.err.println("BEFORE");
	...
	public void checkinBoardWithdoesNotExistUser() {
...
//		try {
		...
		ActionEntity actionEntity = new ActionEntity("2019b.marlenba", elementEntity.getElementId(),
				notExistUserEntity.getUserSmartspace(), notExistUserEntity.getUserEmail(), "CheckIn", new Date(),
